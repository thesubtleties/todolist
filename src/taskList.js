...
const taskList = (() => {
  ...
  const taskFactory = (task) => {
    const newTask = {
      name: task.name, description: task.description, dueDate: task.dueDate, priority: task.priority, timeMade: (task.timeMade) ? task.timeMade : `${fns.format(new Date(), dateRules.dateFormat)}`, isComplete: (task.isComplete) ? task.isComplete : false, project: task.project, id: taskCounter,
    };
    upTaskCount();
    return newTask;
  };
  ...
  const returnTasksByProject = (projectName) => tasks.filter((task) => (task.project === projectName));
  ...
  const removeProjectFromList = (projectToRemove) => {
    const projectNumber = projectList.findIndex((project) => project === projectToRemove);
